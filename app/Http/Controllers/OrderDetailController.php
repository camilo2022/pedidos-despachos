...
class OrderDetailController extends Controller
{
    ...
    {
        try {
            $order = Order::with('seller_user', 'client', 'wallet_user', 'business')->findOrFail($id);            
    ...
    public function store(OrderDetailStoreRequest $request)
    {
        try {
            ...
            if($order->seller_status == 'Aprobado') {
                ...
                foreach($sizes as $size) {
                    $inventories = Inventory::where('product_id', $request->input('product_id'))->where('size_id', $size->id)->where('color_id', $request->input('color_id'))->where('system', 'PROYECCION')->get();
                    $quantity = $request->input("T$size->code");
    ...
    public function authorized(OrderDetailAuthorizeRequest $request)
    {
        try {
            ...
            if(in_array($orderDetail->status, ['Suspendido', 'Cancelado'])) {
                ...
                ->join('sizes', 'sizes.id', 'inventories.size_id')
                ->where('quantity', '>', 0)
                ->whereIn('warehouse_id', User::with('warehouses')->findOrFail($orderDetail->order->seller_user_id)->warehouses->pluck('id')->toArray())
                ...
                ->where('product_id', $orderDetail->product_id)
                ->where('color_id', $orderDetail->color_id)
                ->where('orders.business_id', $orderDetail->order->business_id)
                ->when(in_array($orderDetail->order->seller_user->title, ['VENDEDOR ESPECIAL']),
                    function ($query) {
                        $query->whereIn('orders.seller_status', ['Aprobado'])
                        ->whereIn('orders.wallet_status', ['Pendiente', 'Autorizado'])
                        ->whereIn('order_details.status', ['Pendiente', 'Autorizado'])
                ...
                if(empty($committed)) {
                    ...
                    $color = Color::findOrFail($orderDetail->color_id);
                    $committed->MARCA = $product->trademark;
                    $committed->REFERENCIA = $product->code;
                    $committed->COLOR = $color->name;
                    foreach ($sizes as $size) {
