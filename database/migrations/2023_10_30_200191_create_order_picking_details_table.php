...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('order_picking_details', function (Blueprint $table) {
            $table->id();
            $table->foreignIdFor(OrderPicking::class)->constrained()->onUpdate('cascade')->onDelete('cascade');
            $table->foreignIdFor(OrderDispatchDetail::class)->constrained()->onUpdate('cascade')->onDelete('cascade')->unique();
            ...
            $table->enum('status', ['En curso', 'Cancelado', 'Revision', 'Aprobado'])->default('En curso');
            $table->datetime('date');
            $table->timestamps();
            $table->softDeletes();
        });
    ...
    {
        Schema::dropIfExists('order_picking_details');
    }
};
